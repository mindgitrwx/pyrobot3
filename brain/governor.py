...
class Governor:
    ...
    def map(self, vector):
        ...
        if index in self.histogram:
            self.histogram[index] += 1
        else:
            if index >= 0:
                self.histogram[index] = 1
        if index in self.decayHistogram:
    ...
    def nextItem(self):
        ...
        retval = None
        try:
            retval = next(self.ravq.models.next())
    ...
    def __nextitem__(self):
        """ For use in iterable positions:
            ...
            ...
        """
        return next(self.ravq.models.next())
    ...
    def decayModelVectors(self):
        good = []
        goodNames = []
        for name in list(self.decayHistogram.keys()):
...
if __name__ == '__main__':
    ...
    inputs = []
    for line in sensors:
        inputs.append( [float(x) for x in line.strip().split()])
    targets = []
    for line in locations:
        targets.append( [float(x) for x in line.strip().split()])

...
class VisPsom(psom):
      ...
      for y in range(self.ydim):
         ...
         for x in range(self.xdim):
            ...
            if init:
               ...
               px,py = list(map(int, self.canvas.coords(self.cells[y][x]["traintext"])))
   ...
   def canvas_clicked_up(self, event):
      ...
      for item in celllist:
         if item in list(self.cellhash.keys()):
            cell = item
            break
   ...
   def _train_updatefill(self, curr_pt):
      ...
      for pt in list(self.history.keys()):
         act = self.history[pt]

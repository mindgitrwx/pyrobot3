...
class psom:
    ...
    def init_training(self,alpha_0,radius_0,runlen,errorwindow=1):
        ...
        if(alpha_0 < 0.0 or alpha_0 > 1.0):
            raise PsomError("Invalid learning rate: %s. Alpha must be between 0 and 1.0 \
                  (inclusive)" % alpha_0)
        # check radius of learning effect
        if(radius_0 < 1.0):
            raise PsomError("Invalid radius: %s. Radius of learning effect must be at least 1" \
                  % radius_0)
        # make sure radius of learning effect is not greater than map dimension
        if(radius_0 > self.xdim and radius_0 > self.ydim):
            raise PsomError("Invalid radius: %s. SOM dimension is only %sx%s" \
                  % (radius_0, self.xdim, self.ydim))
        # runlen must be greater than or equal to 1
        if(runlen < 1):
            raise PsomError("Invalid run length: %s. Run length must be at least 1" % runlen)
        
        # error window must be greater than or equal to 1
        if(errorwindow < 1):
            raise PsomError("Invalid size of error window: %s.  Window size must be at least 1" \
                  % errorwindow)
    ...
    def logging_set(self, mode='unset', type='unset', dir='unset', prefix='unset',
        ...
        if(mode != 'unset'):
            if(mode != 'map' and mode != 'train' and mode != 'both'):
                raise PsomError("Unrecognized logging mode: %s" % mode)
            self.log_mode = mode
        if(type != 'unset'):
            if(type != 'file' and type != 'dir'):
                raise PsomError("Unrecognized logging type: %s" % type)
            self.log_type = type
        if(dir != 'unset'):
            if(dir[-1] != "/"):
                dir += "/"
    ...
    def map(self,vector):
        ...
        if(vector.dim != self.dim):
            raise PsomError("Mismatched dimensions of mapping vector (size %s) \
    ...
    def train(self,vector):
        ...
        if(vector.dim != self.dim):
            raise PsomError("Mismatched dimensions of training vector (size %s) \
                  and model vector (size %s)" % (vector.dim, self.dim))
    ...
    def get_activations(self,mode='bubble',radius=1.0):
        ...
        """
        if(self.last == 'unset'):
            raise PsomError("Cannot get activation levels. SOM has not yet been mapped to.")
...
class vector:
	...
	2. To use masking:
        ...
        if(elts != 'unset'):
            ...
            if(weight < 0.0):
                raise VectorError("Invalid weight for training vector: %s. \
            ...
            if(mask != 'NULL'):
                ...
                if (len(mask) != dim):
                    raise VectorError("Mismatched dimensions of vector (len %s) and mask (len %s)" \
                          % (dim,len(mask)))
                # make sure mask is a binary list
                for item in mask:
                    if(item != 0 and item != 1):
                        raise VectorError("Invalid mask: %s.  Mask must be a binary list" % mask)
...
if(__name__ == '__main__'):
    ...
    if 1:
        ...
        for v in vecs:
            ...
            v.display()
            print("maps to model vector", end=' ')
            m.display()
            print("at point", end=' ')
            m.point.display()
        mysom4.logging_off()

...
class Gene:
    ...
    def display(self):
        if self.mode == 'bit' or self.mode == 'integer': 
            ...
            list(map(lambda v: display("%3.2f" % v), self.genotype))
        elif self.mode == 'char':
...
class Population:
    ...
    def statistics(self):
        ...
        if self.verbose > 0:
            ...
            print("Elite fitness:", [x.fitness for x in self.eliteMembers])
            sys.stdout.flush()
...
class GA:
    ...
    def generate(self):
        ...
        """
        newpop = list(range(self.pop.size))
        i = 0
        ...
        elitePositions = [x.position for x in self.pop.eliteMembers]
    ...
    def saveGenesToFile(self, filename, listOfPositions = None):
        ...
        if listOfPositions == None:
            listOfPositions = list(range(len(self.pop.individuals)))
        fp = open(filename, "w")
        if self.verbose > 0:
...
if __name__ == '__main__':
    ...
    print("Do you want to evolve a list of integers to maximize their sum? ", end=' ')
    if sys.stdin.readline().lower()[0] == 'y':
        ...
        print("Press enter to continue evolving...", end=' ')
        sys.stdin.readline()
        ga.evolve()
        print("Press enter to Test init from file (load all with mutate)...", end=' ')
        sys.stdin.readline()
        print("reInitialize pop...")
        ga.reInitialize()
        ga.initGenesFromFile("maxsumga.genes")
        ga.evolve()
        print("Press enter to Test init from file (load 1 no mutate)...", end=' ')
        sys.stdin.readline()
        ga.saveGenesToFile("bestsumga.genes", (ga.pop.bestMember.position,))
        ga.reInitialize()
        ga.initGenesFromFile("bestsumga.genes", 1, 0)
        ga.evolve()
        print("Press enter to Test init from file (load 1, with mutate, full)...", end=' ')
        sys.stdin.readline()
        ga.reInitialize()
    ...
    print("Do you want to evolve a neural network that can do XOR? ", end=' ')
    if sys.stdin.readline().lower()[0] == 'y':
        ga = NNGA(300)
        ga.evolve()
    ...
    print("Do you want to evolve a phrase? ", end=' ')
    if sys.stdin.readline().lower()[0] == 'y':
        ...
        class PhraseGA(GA):
            ...
            def isDone(self):
                print("Best:", end=' ')
                self.pop.bestMember.display()
                #return (phrase == string.join(self.pop.bestMember.genotype, ""))DEBUG
                return (phrase == self.pop.bestMember.genotype.join(""))
    ...
    print("Do you want to play mastermind? ", end=' ')
    if sys.stdin.readline().lower()[0] == 'y':
        ...
        class MasterMindGA(GA):
            ...
            def isDone(self):
                print("Best:", end=' ')
                self.pop.bestMember.display()
                # DEBUG return (phrase == string.join(self.pop.bestMember.genotype, ""))
                # DEBUg return (phrase == string.join(self.pop.bestMember.genotype, ""))
                return (phrase == self.pop.bestMember.genotype.join(""))

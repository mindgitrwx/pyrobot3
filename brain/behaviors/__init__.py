...
class BehaviorBasedBrain(Brain):
   ...
   def getStates(self, status = 1):
      return [state.name for state in list(self.states.values()) if state.status == status]
   ...
   def deactivate(self, name):
      ...
      self.activeState = None
   def add(self, state):
      if state.name in list(self.states.keys()):
   ...
   def step(self):
      ...
      totalTruth = {}
      totalEffects = {}
      for c in list(self.controls.keys()):
         totalTruth[c] = 0.0
         totalEffects[c] = 0.0
      for e in list(self.effectsTotal.keys()): # state:beh:controller
         s, b, c = e.split(':')
...
class State:
   ...
   def getState(self, statename):
      if statename in list(self.brain.states.keys()):
   ...
   def step(self):
      pass # normally will overload
   def add(self, b):
      if b.name in list(self.behaviors.keys()):
   ...
   def run(self):
      for bkey in list(self.behaviors.keys()):

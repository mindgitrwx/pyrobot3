...
def div_func(*operands): 
    ...
    if operands[1] == 0:
        return sys.maxsize # not all python's have "infinity"
    else:
...
class GPTree:
    ...
    def eval(self, env = None):
        if env == None:
            env = share.env
        if self.op not in list(env.operators().keys()):
            ...
            if self.op in list(env.lazyOps().keys()):
                op = env.lazyOps()[self.op].func
                retval = op(*(self.children, env))
            elif self.op in list(env.regularOps().keys()):
                op = env.env[self.op].func
                results = [x.eval(env) for x in self.children]
                retval = op(*results)
            else:
...
class GPGene(Gene):
    def __init__(self, **args):
        ...
        if (random.random() < self.bias):
            terminals = list(share.env.terminals().keys())
            if len(terminals) == 0:
            ...
            operators = list(share.env.operators().keys())
            if len(operators) == 0:
    ...
    for i in range(len(exp)):
        if exp[i] == "(":
            ...
            if current_symbol != '':
                ...
                current_symbol = ''
            current_symbol = objType(*stack.pop(-1))

...
class NNSettingsDialog(tkinter.simpledialog.Dialog):
    def __init__(self, parent, network):
        ...
        tkinter.simpledialog.Dialog.__init__(self, parent, title="Change Network Settings")
        ...
        for paramTuple in self.entryList:
            try:
                ...
                if not boundsCheck(conversion(var)):
                    tkinter.messagebox.showerror(title=key, message=paramTuple[6])
                    return 0
            except ValueError:
                tkinter.messagebox.showerror(title=key, message=paramTuple[6])
                return 0
...
class VisConxBase:
    ...
    def updateHintonWeights(self):
        for diag in list(self.hintonDiags.values()):
            if diag:
                diag.updateDiag()

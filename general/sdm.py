...
class SDM:
    ...
    def select(self, address, threshold = None):
        ...
        if self.verbose: print("threshold = %d" % threshold, end=' ')
        retval = [HardLocation.select(m, address, threshold) for m in self.memory]
        return retval
    ...
    def write(self, address, data, threshold = None):
        if self.verbose: print("Writing", data, end=' ')
        s = self.select( address, threshold)
        for i in range(len(s)):
            if s[i]:
    ...
    def distance(self, address):
        retval = [HardLocation.distance(m, address) for m in self.memory]
        return retval
...
if __name__ == '__main__':
    ...
    #print sdm.select(ra)
    print(sdm.read(ra))
    print(sdm.write(ra, list(map(rnd2, randomBits(sdm.dataLength)))))
    print(sdm.read(ra))

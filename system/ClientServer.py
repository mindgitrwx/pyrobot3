...
#		SocketServer.TCPServer.serve_forever(self)
	...
	def process_request(self, request, client_address):
		...
		"""
		_thread.start_new_thread(self.finish_request, (request, client_address))
...
class Client:
	...
	def connect(self):
		...
		if (shift>10):
			console.log(console.FATAL,'Client can\'t connect:' + repr(self.host + ":"\
				+ str(self.port)))
			self.alive = 0
		else:				
			console.log(console.INFO,'Client connected to ' + repr(self.host + ":"\
				+ str(self.port)))
	...
	def send(self, smsg):#smsg - Message
		...
		"""
		#pickle it
		data = pickle.dumps(smsg,0)#0 means ASCII encryption

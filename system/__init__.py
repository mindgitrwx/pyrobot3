...
#import PyrobotRobot60000 as userspace
...
def loadINIT(filename, engine=0, redo=0, brain=0, args=None):
    #DEBUG : added userspace
    #userspace = ''

    path = filename.split("/")
    modulefile = path.pop() # module name
    module = modulefile.split(".")[0]
    #search = string.join(path, "/") #DEBUG
    search = '/'.join(path)
    oldpath = sys.path[:] # copy
    sys.path.insert(0, search)
    print("Attempting to import '%s'..." % module)
    print("module name.. '%s'..." % module)  # DEBUG
    print("exec : import " + module + " as userspace") #DEBUG
    #importUserspace = "import " + module + " as userspace"
    #print(importUserspace)
    #DEBUG exec("import " + module + " as userspace")
    #exec("print('just test about exec function')")
    #exec(importUserspace)
    #exec ("import %s as userspace" % module)
    userspace = importlib.import_module(module)
    # DEBUG
